import { extractAllCollections } from "@sjh/shopify";
...
async function main(): Promise<void> {
  const options: Parameters<typeof extractAllCollections>[0] = {
  ...
  const result = await extractAllCollections(options);

  console.log(JSON.stringify(result, null, 2));
}

main().catch((error: unknown) => {
  console.error(error instanceof Error ? error.message : "Shopify bulk collection extraction failed.");
  process.exitCode = 1;
});
